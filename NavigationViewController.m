...
#pragma mark - Initialization 定位标识符号
...
- (void)configMapView
{
    ...
    [self.mapView setZoomLevel:16.1 animated:YES];//缩放级别
}
...
#pragma mark - MAMapView Delegate 设置标注样式
...
- (MAAnnotationView *)mapView:(MAMapView *)mapView viewForAnnotation:(id<MAAnnotation>)annotation
{
    if ([annotation isKindOfClass:[NavPointAnnotation class]])
    {
        ...
        pointAnnotationView.animatesDrop   = NO;  //设置气泡可以弹出，默认为NO
        pointAnnotationView.canShowCallout = NO;  //设置标注动画显示，默认为NO
        pointAnnotationView.draggable      = NO;  //设置标注可以拖动，默认为NO
        ...
        pointAnnotationView.rightCalloutAccessoryView = [UIButton buttonWithType:UIButtonTypeDetailDisclosure];
        pointAnnotationView.pinColor                  = [self.annotations indexOfObject:annotation];
